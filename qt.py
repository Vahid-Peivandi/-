#!/usr/bin/python
# -*- coding: utf- -*-
#shebang name khat bala hast ke baesse mishe dar sorate moshkell az address jari run beshe
import time
...
#barayeh greftan vorodi reshteh be kar mireh
   ...
   b1=QPushButton(win)
   #esme dokmeh b1 ro vaghte feshar dadan anjam mishe nesbat mideh be panjreh win
...
#mokhtasat dokmeh
   ...
   b2 = QPushButton(win)
   #esme dokmeh b2 ro vaghte feshar dadan anjam mishe nesbat mideh be panjreh win
   b2.setText(u"نوشتن فایل مرتب شده")
   #gozashtan name barayeh dokmeh
   b2.move(355,140)
   #mokhtasat dokmeh
   QObject.connect(b2,SIGNAL("clicked()"),b2_clicked)
#vaghti roydadi etefagh mioftad signali fresetadeh va codi ejra mishe, argoman aval name objecti hast ke signal ro mifresteh, argoman dovom signal hast, sevomin argoman codi ke ejra midhe hast
   b3 = QPushButton(win)
      #esme dokmeh b3 ro vaghte feshar dadan anjam mishe nesbat mideh be panjreh win
...
#tarif moteghayereh b2 vaghti click mishe
   f = open("sorted",'w')
   #fileli ra ba name morede nazar baz mikoneh va ba dastrsiyeh neveshtan minviseh
   f.writelines(["%s\n" % item  for item in content])
#fileli ke moratab shode khat be khat neveshteh mishe ke reshteh ham ghabool mikoneh
   f.close()
#file basteh mishe
